...
if __name__ == '__main__':
    ...
    # model_param = torch.load("path/to/weight.pth", map_location=device)
    # model.load_state_dict(model_param)

    train_dataset = Custom27Dataset(root_dir=dataset, split='train', 
                                     center_and_scale=False, normalize=True, random_rotate=False,
                                     num_threads=8)
    graphs = train_dataset.graphs() # no need to load graphs again !
    val_dataset = Custom27Dataset(root_dir=dataset, graphs=graphs, split='val', 
